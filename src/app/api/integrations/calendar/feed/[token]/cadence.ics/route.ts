...
import {
  ...
  ICS_FEED_LOOKAHEAD_DAYS,
  ICS_FEED_LOOKBACK_DAYS,
  shiftIsoDate,
...
export async function GET(
  ...
  const profileResponse = await admin
    .from("profiles")
    .select("calendar_feed_token_version, timezone")
    .eq("id", userId)
    .maybeSingle();
  ...
  const windowStart = shiftIsoDate(localToday, -ICS_FEED_LOOKBACK_DAYS);
  const windowEnd = shiftIsoDate(localToday, ICS_FEED_LOOKAHEAD_DAYS);
  ...
  const etag = createPlannerFeedEtag(feedItems);
