...
describe("social duo planner proposals route", () => {
  beforeEach(() => {
  ...
  it("lists planner proposals", async () => {
    const response = await GET(
      new Request("http://localhost/api/social/duo/planner-proposals?scopeMonth=2026-08")
    );
  ...
  it("creates planner proposal", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/social/duo/planner-proposals", {
        method: "POST",
