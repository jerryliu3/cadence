...
export async function POST(
  ...
  try {
    ...
    if (!proposal) {
      throw new ApiRouteError(404, "proposal_not_found", "Planner proposal was not found.");
    }
    if (proposal.targetOwnerId !== socialContext.userId) {
      throw new ApiRouteError(403, "proposal_not_target_owner", "Only the target owner can accept.");
    }
    if (proposal.status !== "pending") {
      throw new ApiRouteError(409, "proposal_not_pending", "Planner proposal is no longer pending.");
    }
    ...
    if (digestError) {
      throw new ApiRouteError(500, "planner_digest_unavailable", "Planner digest is unavailable.", {
    ...
    if (itemsError) {
      throw new ApiRouteError(500, "planner_items_unavailable", "Planner items are unavailable.", {
    ...
    if (publishError) {
      throw new ApiRouteError(409, "planner_proposal_apply_failed", "Could not apply planner proposal.", {
    ...
    if (resolveError || !resolved) {
      throw new ApiRouteError(
        500,
        "planner_proposal_resolve_failed",
        "Proposal could not be marked accepted.",
        {
    ...
    if (error instanceof z.ZodError) {
      return apiErrorResponse(
        new ApiRouteError(400, "validation_failed", "Proposal payload failed validation.", {
          issues: error.issues,
        }),
        correlationId
      );
    ...
    return apiErrorResponse(
      new ApiRouteError(
        500,
        "internal_error",
        "Planner proposal accept request failed unexpectedly."
      ),
      correlationId
    );
  }
