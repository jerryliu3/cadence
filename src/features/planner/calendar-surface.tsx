...
type PlannerPreviewWorkUnit = NonNullable<
  PlannerContextPayload["preview"]
>["workUnits"][number];

type DraftMoveValidationResult =
  | {
      ok: true;
      normalizedDate: string;
      occupancyKey: string;
      baselineUnit: PlannerPreviewWorkUnit;
    }
  | {
      ok: false;
      code:
      ...
      creditEnd?: string;
      conflictScheduledDate?: string | null;
    };
...
export function CalendarSurface({
  ...
  const scopedDraftCommands = useMemo(
    () => sortPlannerDraftCommands(scopeDraftCommands),
    [scopeDraftCommands]
  );
  ...
  const effectiveDraftCommands = useMemo(
    () =>
      scopedDraftCommands.filter((command) =>
        effectivePreviewEntryKeys.has(draftCommandEntryKey(command))
      ),
    [effectivePreviewEntryKeys, scopedDraftCommands]
  );
  ...
  const refreshDraftPreview = async (
    ...
    solveIntent: "stable" | "replan" = "stable"
  ) => {
  ...
  const buildDraftSnapshotToken = (commands: PlannerDraftCommand[]) =>
  ...
  const validateDraftMove = useCallback(
    ({
      entry,
      nextDate,
      occupancyByGoalDate,
      ...
      entry: PlannerDayDetailEntry;
      nextDate: string;
      occupancyByGoalDate: Map<string, Set<string>>;
    }): DraftMoveValidationResult => {
      if (entry.draftGhost) {
        return { ok: false, code: "ghost" };
      }
      ...
      if (isEntryImmovableForDraft(entry)) {
        return { ok: false, code: "immovable" };
      }
      ...
      if (normalizedDate < moveWindow.start || normalizedDate > moveWindow.end) {
        return {
          ok: false,
          code: "outside_window",
          date: normalizedDate,
          window: moveWindow,
          creditEnd: baselineUnit.creditWindow?.end ?? moveWindow.end,
        };
      }
      const occupancyKey = `${entry.originalGoalId}:${normalizedDate}`;
      const conflictKeys = occupancyByGoalDate.get(occupancyKey);
      if (
        conflictKeys &&
      ...
      const completionFactConflict = (
        completionFactUnitsByGoalDate.get(occupancyKey) ?? []
      ...
      if (completionFactConflict) {
        return {
          ...
          date: normalizedDate,
          conflictScheduledDate: completionFactConflict.scheduledDate ?? null,
        };
      }
      return {
        ok: true,
    ...
    [completionFactUnitsByGoalDate, previewUnitByEntryKey]
  );

  const formatCoachMoveRejection = (
    command: Extract<PlannerDraftCommand, { kind: "move_item" }>,
    rejection: Exclude<DraftMoveValidationResult, { ok: true }>
  ) => {
    switch (rejection.code) {
      case "invalid_date":
        return `Skipped move for ${command.goalId}/${command.unitKey} because the target date was invalid.`;
      case "immovable":
        return `Skipped move for ${command.goalId}/${command.unitKey} because completed or historical sessions cannot be moved.`;
      case "unavailable":
        return `Skipped move for ${command.goalId}/${command.unitKey} because the item is unavailable in the current preview.`;
      case "no_window":
        return `Skipped move for ${command.goalId}/${command.unitKey} because the item has no movable placement window.`;
      case "outside_window":
        return `Skipped move for ${command.goalId}/${command.unitKey} because the target date was outside its move window.`;
      case "collision":
        return `Skipped move for ${command.goalId}/${command.unitKey} because another session for that goal already exists on ${rejection.date}.`;
      case "completion_conflict":
        return `Skipped move for ${command.goalId}/${command.unitKey} because that goal already has a completion fact on ${rejection.date}.`;
      case "ghost":
      default:
        return `Skipped move for ${command.goalId}/${command.unitKey} because only concrete planner sessions can be edited.`;
    }
  };

  const applyCoachDraftCommands = (commands: PlannerDraftCommand[]) => {
    ...
    for (const [day, entries] of entriesByDate.entries()) {
      for (const entry of entries) {
        ...
        const existing = occupancyByGoalDate.get(key) ?? new Set<string>();
        existing.add(entry.key);
        occupancyByGoalDate.set(key, existing);
      }
    }
    const recordRejection = (message: string) => {
      rejectedCount += 1;
      if (!rejectionMessages.includes(message)) {
    ...
    for (const command of commands) {
      ...
      if (!entry) {
        recordRejection(
          `Skipped ${command.kind} for ${command.goalId}/${command.unitKey} because the item is not available in this preview scope.`
        );
        continue;
      }
      if (entry.draftGhost) {
        recordRejection(
          `Skipped ${command.kind} for ${command.goalId}/${command.unitKey} because only concrete planner sessions can be edited.`
        );
        continue;
      }

      if (command.kind === "move_item") {
        const nextDate = command.scheduledDate;
        if (!nextDate) {
          recordRejection(
            `Skipped move for ${command.goalId}/${command.unitKey} because a target date was required.`
          );
          continue;
        }
        if (!scopeMonth) {
        ...
        if (!moveValidation.ok) {
          recordRejection(formatCoachMoveRejection(command, moveValidation));
          continue;
        }
        const { baselineUnit, occupancyKey, normalizedDate } = moveValidation;
        ...
        const previousDay = entryDayByKey.get(entry.key) ?? baselineUnit.scheduledDate;
        if (previousDay) {
          const previousOccupancyKey = `${entry.originalGoalId}:${previousDay}`;
          const existing = occupancyByGoalDate.get(previousOccupancyKey);
          if (existing) {
            existing.delete(entry.key);
            if (existing.size === 0) {
              occupancyByGoalDate.delete(previousOccupancyKey);
            }
          }
        }
        const nextOccupancy = occupancyByGoalDate.get(occupancyKey) ?? new Set<string>();
        nextOccupancy.add(entry.key);
        occupancyByGoalDate.set(occupancyKey, nextOccupancy);
        appliedCount += 1;
        continue;
      }
      ...
      if (nextPlan.status === "blocked") {
        recordRejection(
          nextPlan.reason === "invalid_time"
            ? `Skipped time update for ${command.goalId}/${command.unitKey} because the time was invalid.`
            : `Skipped time update for ${command.goalId}/${command.unitKey} because completed or historical sessions cannot be edited.`
        );
        continue;
      }
  ...
  const coach = usePlannerCoach({
    ...
    effectiveDraftCommands: scopedDraftCommands,
    hasDraftSession,
    refreshDraftPreview,
  ...
  const queueDraftMoveCommand = useCallback(
    ({
      ...
      const moveValidation = validateDraftMove({
        entry,
        nextDate,
        occupancyByGoalDate: moveConflictByGoalDate,
      });
      if (!moveValidation.ok) {
        switch (moveValidation.code) {
          case "ghost":
          ...
          case "no_window":
            ...
            break;
          case "outside_window": {
            const moveWindow = moveValidation.window;
            const creditWindowEnd = moveValidation.creditEnd;
            const normalizedDate = moveValidation.date;
            if (
              moveWindow &&
              normalizedDate &&
              normalizedDate < moveWindow.start
            ) {
              toast.error(
              ...
              creditWindowEnd &&
              normalizedDate &&
              normalizedDate > creditWindowEnd
            ) {
              toast.error(
              ...
              toast.error("That date is outside this session's allowed planner window.");
            }
            break;
          }
          case "collision":
          ...
          case "completion_conflict":
            if (
              moveValidation.conflictScheduledDate &&
              moveValidation.date &&
              moveValidation.conflictScheduledDate !== moveValidation.date
            ) {
              toast.error(
                `That goal is already marked done on ${moveValidation.date} (credited from the ${moveValidation.conflictScheduledDate} session).`
              );
            } else {
      ...
      if (source === "drag_drop") {
        toast.success(
          `Moved ${getEntryDisplayTitle(entry)} in preview mode to ${normalizedDate}.`
        );
      }
      return true;
    },
      ...
      try {
        payload = await postJson<PlannerErrorPayload & { replayed?: boolean }>(
          "/api/planner/save",
          {
            ...
            solveIntent: effectivePreview.solveIntent,
          }
        );
      } catch (error) {
