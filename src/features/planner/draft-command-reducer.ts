...
export function draftCommandReducer(
  ...
  if (action.type === "replace_all") {
    const retained = state.commands.filter(
      (command) => command.scopeMonth !== action.scopeMonth
    );
    const replaced = [...action.commands]
      .sort((left, right) => left.sequence - right.sequence)
      .map((command) => ({
        scopeMonth: action.scopeMonth,
        command,
      }));
    const commands = [...retained, ...replaced];
    const maxSequence = commands.reduce(
      (max, command) => Math.max(max, command.command.sequence),
      0
    );
    return {
      commands,
      nextSequence: Math.max(state.nextSequence, maxSequence),
    };
  }
