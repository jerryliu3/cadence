...
import {
  acceptDuoPlannerProposal,
  fetchDuoPartnerPlan,
  fetchDuoPlannerProposals,
  rejectDuoPlannerProposal,
  withdrawDuoPlannerProposal,
} from "@/features/social/data";
import { PlannerProposalForm } from "@/features/social/duo/planner-proposal-form";
import type { DuoPartnerPlanItem, DuoPlannerProposal } from "@/features/social/types";
...
export function PartnerPlan({ partnerId }: { partnerId: string }) {
  ...
  const [items, setItems] = useState<DuoPartnerPlanItem[]>([]);
  const [proposals, setProposals] = useState<DuoPlannerProposal[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [plan, proposalResponse] = await Promise.all([
        fetchDuoPartnerPlan(scopeMonth),
        fetchDuoPlannerProposals(scopeMonth),
      ]);
      setItems(plan.items);
  ...
  async function handleAccept(proposalId: string) {
    try {
      await acceptDuoPlannerProposal(proposalId);
      await load();
    } catch (acceptError) {
  ...
  async function handleReject(proposalId: string) {
    try {
      await rejectDuoPlannerProposal(proposalId);
      await load();
    } catch (rejectError) {
  ...
  async function handleWithdraw(proposalId: string) {
    try {
      await withdrawDuoPlannerProposal(proposalId);
      await load();
    } catch (withdrawError) {
