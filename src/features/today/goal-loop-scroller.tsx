...
export function GoalLoopScroller({ goals, renderGoal }: GoalLoopScrollerProps) {
  ...
  return (
    <div className="relative rounded-2xl border bg-card/60 p-2">
      <div
        ref={scrollerRef}
        data-testid="goal-loop-scroller"
        data-no-swipe="true"
        className="h-[min(65vh,560px)] overflow-y-scroll overscroll-contain rounded-xl pr-4 [scrollbar-width:thin] [touch-action:pan-y]"
        onScroll={onScroll}
      >
