...
export function buildCoachPrompt({
  ...
  return [
    ...
    "When the user asks for calendar changes, describe the intended schedule in proposal.calendarIntent; the server compiles it deterministically into safe policy patches and draft item commands.",
    "",
    ...
    "- action=apply when the user asks for planner edits that can be represented as global policy edits and/or item-level session edits.",
    "- For action=apply, global can be null or include restWeekdays/addBlackoutRanges/removeBlackoutRanges.",
    "- For action=apply, items can include one or more existing planner sessions by goalId+unitKey with any combination of scheduledDate, label, and localTime.",
    ...
    "The calendar compiler cannot create goals, invent sessions, or repurpose an unrelated goal. Item edits must map to existing goalId + unitKey rows. State this limitation honestly.",
