...
-- Extend the inner publish function to include time columns in its INSERT,
-- avoiding a post-INSERT UPDATE that would trip the immutability guard.
...
-- The eligibility-mode wrapper just delegates; no post-INSERT UPDATE needed.
create or replace function public.publish_execution_plan_service(
  p_owner uuid,
  p_scope_month date,
