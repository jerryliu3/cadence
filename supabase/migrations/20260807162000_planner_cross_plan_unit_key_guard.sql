create or replace function private.enforce_cross_plan_goal_unit_conflict()
returns trigger
language plpgsql
set search_path = ''
as $$
...
begin
  if new.requirement_kind not in ('milestone_sequence', 'deadline_total') then
    return new;
  end if;
  ...
  if v_new_original_goal_id is null then
    raise exception using
      errcode = '23514',
      message = 'cross_plan_goal_unit_conflict_context_missing';
  end if;
  ...
  if exists (
    ...
    where item.owner_id = new.owner_id
      and item.unit_key = new.unit_key
      and item.requirement_kind in ('milestone_sequence', 'deadline_total')
    ...
    raise exception using
      errcode = '23514',
      message = 'cross_plan_goal_unit_conflict';
  end if;
...
drop trigger if exists execution_plan_items_cross_plan_goal_unit_guard
on public.execution_plan_items;

create trigger execution_plan_items_cross_plan_goal_unit_guard
before insert
on public.execution_plan_items
for each row
execute function private.enforce_cross_plan_goal_unit_conflict();
