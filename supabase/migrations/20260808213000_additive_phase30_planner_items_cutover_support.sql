-- Additive Phase 30:
-- Keep planner_items as the canonical planner write surface for cutover.
-- Preserve original_scheduled_date across publishes and add stale-digest guardrails
-- for lock updates.
...
begin
  ...
  insert into tmp_schedule_input (
    goal_id,
    unit_key,
    scheduled_date,
  ...
  create temp table tmp_existing_original (
    goal_id uuid not null,
    unit_key text not null,
    original_scheduled_date date,
    scheduled_date date not null
  ...
  insert into tmp_existing_original (
    goal_id,
    unit_key,
    original_scheduled_date,
    scheduled_date
  ...
  begin
    ...
    select
      ...
      item.locked
    from tmp_schedule_input item
    left join tmp_existing_original existing
...
as $$
  select *
  from public.set_planner_item_lock(
    p_item_id,
    p_locked,
    public.get_planner_schedule_digest(auth.uid())
  );
$$;

grant execute on function public.set_planner_item_lock(uuid, boolean, text) to authenticated;
