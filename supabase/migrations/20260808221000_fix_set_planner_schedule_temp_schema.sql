-- Fix CI/prod function lint/runtime compatibility for set_planner_schedule.
-- Keep SECURITY DEFINER hardening with empty search_path and avoid temp-table
-- relation checks that fail Supabase db lint static analysis.
...
begin
  ...
  if exists (
    ...
    select 1
    from parsed_input
    where goal_id is null or unit_key is null or scheduled_date is null
  ) then
    raise exception using errcode = '22023', message = 'invalid_schedule_payload';
  end if;
  ...
  if exists (
    ...
    raise exception
      using errcode = '22023', message = 'scheduled_date_outside_scope_month';
  end if;
  ...
  if exists (
    ...
    raise exception
      using errcode = 'P0001', message = 'scheduled_outside_goal_lifetime';
  end if;
  ...
  if exists (
    ...
    existing_other_month as (
      ...
      where item.owner_id = v_owner
        ...
        and not exists (
          select 1
          from parsed_input incoming_item
