-- Allow chained planner moves to publish.
--
-- `planner_items_goal_date_unique` was non-deferrable, so publishing a run of
-- items shifted along by one day failed on the first row: the date it moves
-- onto is still held by the item that is about to move off it. The final state
-- was always unique -- set_planner_schedule validates that before writing --
-- only an intermediate row state was not.
--
-- This surfaced once a coach policy change could move several sessions at once;
-- a single move to an empty date never collided, which is why it looked
-- coach-specific.
...
begin
  ...
  begin
    -- A run of chained moves transiently collides: shifting an item onto the
    -- date the next item has not vacated yet trips the per-row check, even
    -- though the final state is unique. Ordering the writes cannot fix it in
    -- general because two items can swap dates. The payload is already
    -- validated for duplicate (goal_id, scheduled_date) above, so defer the
    -- constraint across the write and force the check back inside this block,
    -- where a genuine duplicate still surfaces as schedule_conflict rather than
    -- a raw unique_violation at commit.
