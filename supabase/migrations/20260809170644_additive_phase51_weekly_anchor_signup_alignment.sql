-- Additive phase 51:
-- Keep weekly anchor cutover aligned for newly-created profiles.

alter table public.profiles
alter column weekly_anchor_effective_on set default (
  current_date + (
    (
      7 - (
        (
          extract(dow from current_date)::int - 1 + 7
        ) % 7
      )
    ) % 7
  )
...
declare
  ...
  resolved_weekly_anchor_effective_on date;
begin
  base_username := lower(
    coalesce(
  ...
  resolved_weekly_anchor_effective_on := (
    current_date + (
      (
        7 - (
          (
            extract(dow from current_date)::int - resolved_week_starts_on + 7
          ) % 7
        )
      ) % 7
    )
