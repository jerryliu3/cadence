...
begin
  if not exists (
    ...
    from pg_type
    where typnamespace = 'public'::regnamespace
      and typname = 'goal_partner_visibility'
  ) then
...
set social_discoverable = true
where social_discoverable is null;
...
where social_leaderboard_eligible is null;

update public.profiles
set social_challenge_eligible = true
where social_challenge_eligible is null;
...
alter table public.profiles
  alter column social_discoverable set default true,
  alter column social_activity_visible set default true,
  alter column social_leaderboard_eligible set default true,
  alter column social_challenge_eligible set default true,
  alter column social_visibility_updated_at set default pg_catalog.now();

alter table public.profiles
  alter column social_discoverable set not null,
  alter column social_activity_visible set not null,
  alter column social_leaderboard_eligible set not null,
  alter column social_challenge_eligible set not null,
...
update public.goals
set partner_visibility = 'shared'::public.goal_partner_visibility
where partner_visibility is null;

alter table public.goals
  alter column feed_visibility set default 'private'::public.goal_feed_visibility,
  alter column partner_visibility set default 'shared'::public.goal_partner_visibility;

alter table public.goals
  alter column feed_visibility set not null,
  alter column partner_visibility set not null;
...
begin
  ...
  where profile.id <> v_uid
    and profile.social_discoverable = true
