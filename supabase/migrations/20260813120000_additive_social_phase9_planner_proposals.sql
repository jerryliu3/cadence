-- Additive forward port of social phase 9 planner proposals (PR 151).
-- Original timestamps 20260809210945 / 20260809211252; applied after current main.
...
create unique index if not exists planner_proposals_one_pending
  on public.planner_proposals (team_id, target_owner_id, scope_month)
...
create or replace function public.get_team_partner_plan_service(
  p_scope_month date
)
returns table (
  item_id uuid,
...
begin
  ...
  returning proposal.proposer_id, proposal.target_owner_id, proposal.team_id
  into v_proposer_id, v_target_owner_id, v_team_id;

  if not found then
    return false;
  end if;

  if private.partner_notifications_allowed(v_team_id, v_proposer_id) then
    perform private.enqueue_notification_outbox(
...
revoke all on function public.get_team_partner_plan_service(date)
  from public, anon;
grant execute on function public.get_team_partner_plan_service(date)
  to authenticated;
