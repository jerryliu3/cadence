-- Add a dedicated preparation boundary for reconciling multiple bounded planner
-- windows under one owner lock and one owner-wide digest check. This is not a
-- generic batch mutation surface: p_items is one complete snapshot spanning the
-- ordered windows in p_windows.
...
begin
  ...
  with schedule_input as (
    select
      ...
      nullif(pg_catalog.btrim(row.scheduled_time), '')
        as scheduled_time,
  ...
  validation_flags as (
    select
        ...
        where goal.owner_id = v_owner
          ...
          and (
            ...
            or (
              ...
              and coalesce(goal.target_count, 0) > 0
              and item.unit_key !~ '^total:[1-9][0-9]*$'
            )
            or (
              goal.frequency_type = 'recurring'
              and coalesce(goal.target_count, 0) <= 0
              and item.unit_key !~ '^cadence:[0-9]{4}-[0-9]{2}-[0-9]{2}$'
            )
          )
      ...
      exists (
        ...
        existing_outside_windows as (
          ...
          where existing.owner_id = v_owner
            ...
            and not exists (
              ...
              where incoming_item.goal_id = existing.goal_id
                and incoming_item.unit_key = existing.unit_key
            )
            and not (
              existing.scheduled_date >= current_date
              and (
                goal.is_deleted
                or existing.scheduled_date < goal.start_date
                or (
                  goal.end_date is not null
                  and existing.scheduled_date > goal.end_date
                )
                or (
                  goal.frequency_type = 'fixed_milestones'
                  and (
                    existing.unit_key !~ '^milestone:[1-9][0-9]*$'
                    or (
                      existing.unit_key ~ '^milestone:[1-9][0-9]*$'
                      and substring(
                        existing.unit_key from '^milestone:([1-9][0-9]*)$'
                      )::numeric > coalesce(goal.target_count, 0)
                    )
                  )
                )
                or (
                  ...
                  and coalesce(goal.target_count, 0) > 0
                  and (
                    existing.unit_key !~ '^total:[1-9][0-9]*$'
                    or (
                      existing.unit_key ~ '^total:[1-9][0-9]*$'
                      and substring(
                        existing.unit_key from '^total:([1-9][0-9]*)$'
                ...
                or (
                  goal.frequency_type = 'recurring'
                  and coalesce(goal.target_count, 0) <= 0
                  and existing.unit_key
                    !~ '^cadence:[0-9]{4}-[0-9]{2}-[0-9]{2}$'
                )
              )
            )
        ...
        left join existing_outside_windows existing
  ...
  stale_future as (
    ...
    join public.goals goal on goal.id = item.goal_id
    where item.owner_id = v_owner
      and item.scheduled_date >= current_date
      and (
        goal.is_deleted
        or item.scheduled_date < goal.start_date
        or (goal.end_date is not null and item.scheduled_date > goal.end_date)
        or (
          goal.frequency_type = 'fixed_milestones'
          and (
            item.unit_key !~ '^milestone:[1-9][0-9]*$'
            or (
              item.unit_key ~ '^milestone:[1-9][0-9]*$'
              and substring(
                item.unit_key from '^milestone:([1-9][0-9]*)$'
