...
add column if not exists coverage_fingerprint text not null default '';
...
begin
  ...
  if exists (
    ...
    where pg_catalog.jsonb_typeof(payload.row) <> 'object'
      ...
      or (
        payload.row ? 'coverage_fingerprint'
        and pg_catalog.jsonb_typeof(payload.row -> 'coverage_fingerprint') <> 'string'
      )
      or (
  ...
  with unplaceable_input as (
    select
      ...
      coalesce(pg_catalog.btrim(row.coverage_fingerprint), '') as coverage_fingerprint,
      row.policy_revision,
